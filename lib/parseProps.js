...
function parseSingleProp(prop, options = {}) {
  const isTypeOnlyEnum = _.startsWith(prop, '(');
  const propPair = isTypeOnlyEnum ? [prop] : prop.split(/:\s*([^:]+|!?{.+|\(.+)$/, 2).map(v => v.trim());
  ...
  const {
    isRef, isEnum, isNestedObject, isCollection,
    collectionDimension, isValid, type: extractedType, format,
  ...
  if (isCollection) {
    for (let i = 0; i < collectionDimension; i += 1) {
      def = {
        type: 'array',
        items: def,
      };
...
function parseType(typeDef) {
  ...
  const collectionDimension = isCollection ? typeDef.match(/\[]/g).length : null;
